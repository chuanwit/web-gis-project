...
**当前版本：v10** | 技术栈：Vue 3 · Mapbox GL · AntV L7 · Pinia · G2 · Express · DeepSeek
...
| 交通风险诊断 | 风险区域 pitch=60 | 区域 3D 挤出 + 风险着色 | 风险贡献对比 + 事件类型分布 | 区域风险排行 + 高风险事件列表 |
...
- **初始城市视角**：地图启动即城市俯瞰（zoom=14, pitch=70），无需加载后二次飞行
- **地球视角图层策略**：缩放至地球视角（zoom<5）自动隐藏 L7 图层，避免 globe 旋转导致图层坐标漂移，回到城市视角自动恢复（v10）
...
| 统计图表 | AntV G2 | ^5.4.8 | 柱状图、饼图、玫瑰图（分组柱状图做风险对比） |
...
浏览器打开 `http://localhost:5173/`（端口号以终端输出为准，Vite 端口被占用会自动递增）。
...
│   └── route-test/                    # 路径规划测试
│
├── deploy/                            # 服务器部署脚本（Nginx + PM2）
│   ├── deploy.sh                      # 一键部署脚本（前端 + AI 服务）
│   └── nginx-smart-city.conf          # Nginx 配置（静态托管 + API 代理 + SSE 禁缓冲）
│
├── public/                            # 静态资源
│   ├── favicon.svg
│   └── icons.svg
...
│   │   │       ├── RiskRadarChart.vue     # 风险贡献对比（分组柱状图）
...
- **左侧**：风险贡献对比（分组柱状图，事件数/平均等级/风险分/拥堵指数四维度归一化对比） + 事件类型分布玫瑰图
...
8. **地球视角隐藏 L7 图层**：L7 图层在 Mapbox globe 旋转动画期间与底图坐标系不同步（道路/区域会漂移），`SmartCity/index.vue` 监听 `zoom` 事件，`zoom < 5` 时 `scene.getLayers().forEach(l => l.hide())`，回到城市视角全量 `show()` 后按业务模块轻量校正（不调 `applyBusinessModule`，避免其内部 `flyTo` 打断用户缩放）。（v10）

9. **SSE 代理不缓冲**：vite proxy 转发 `/api/ai` 时需确保不缓冲 SSE 流，`X-Accel-Buffering: no` 头防止 nginx 反代缓冲。

10. **资源清理**：所有组件在 `onBeforeUnmount` 中清理事件监听、销毁地图实例、清除定时器、移除临时图层。

11. **路径规划路网碎片化**：mock 道路在交叉口坐标不共享，`routeGraph.js` 采用空间网格连接边缝合碎片（250m 阈值），优先吸附到大连通分量。
